...
func (s *Server) StartComsumer(que string, num int) error {
	meth := new(JsonMethod)
	for i := 0; i < num; i++ {
		if err := s.regMethod(s.transport.ch, que, meth); err != nil {
			return err
		}
	}
	return nil
...
func (s *Server) setup(ch *amqp.Channel) error {
	for _, r := range s.registries {
		for name, meth := range r.GetMethods() {
			consumerNum := 1
			if cfgNum := viper.GetInt("mq.consumer_num"); cfgNum != 0 {
				consumerNum = cfgNum
			}
...
func (s *Server) regMethod(ch *amqp.Channel, name string, meth Method) error {
	...
	s.queues[q.Name] = append(s.queues[q.Name], q)
